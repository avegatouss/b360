...
final class LoginRedirector
{
    ...
    {
        if (TeamContext::isSuperAdmin($user)) {
            $root = Instance::root()->active()->firstOrFail();

            return [
                'count'     => 1,
                'instances' => [['id' => (int) $root->id, 'slug' => $root->slug]],
            ];
        }
    ...
    {
        if ($instance->isRoot() && ! TeamContext::isSuperAdmin($user)) {
