...
final class FeatureGatingTest extends TestCase
{
    ...
    {
        // Unit-level: FeatureGate::has() returns true for free features
        // even when no instance is set (fallback path).
    ...
    public function test_feature_gate_service_returns_free_features_without_instance(): void
    {
        // FeatureGate::has() falls back to FREE_FEATURES constants when no instance is set.
        CurrentInstance::clear();

        $gate = app(FeatureGate::class);

        $this->assertTrue($gate->has('products.crud'));
        $this->assertTrue($gate->has('sales.basic'));
        $this->assertFalse($gate->has('hr'));
        $this->assertFalse($gate->has('channels'));
    }
}
