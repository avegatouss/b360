<x-dashboard::layouts.master
    :title="'CinetPay - ' . ($instance->name ?? $instance->slug ?? 'B360')"
    :instance="$instance"
    pageTitle="Parametres CinetPay">

<div class="page-header">
    <div class="add-item d-flex">
        <div class="page-title">
            <h4 class="fw-bold">Passerelle CinetPay</h4>
            <h6>{{ $instance->name }} &mdash; Configuration du paiement mobile</h6>
        </div>
    </div>
    <ul class="table-top-head">
        <li>
            <a data-bs-toggle="tooltip" data-bs-placement="top" title="Reduire" id="collapse-header">
    ...
    <div class="col-xl-7">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white border-bottom d-flex align-items-center gap-3">
                ...
                <div>
                    <h5 class="mb-0">Parametres CinetPay</h5>
                    <small class="text-muted">Mobile Money, Orange Money, MTN Money, Carte</small>
                </div>
            </div>
            <div class="card-body">
                <form action="{{ route('eshop360.cinetpay.settings.update', $instance->slug ?? '') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="merchant_id" class="form-control @error('merchant_id') is-invalid @enderror"
                               ...
                               placeholder="Votre identifiant marchand CinetPay">
                        @error('merchant_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        <div class="form-text">Obtenu lors de l'inscription sur CinetPay.</div>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Cle secrete <span class="text-danger">*</span></label>
                        ...
                        <div class="form-text">Ne partagez jamais cette cle.</div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-text">Utilisez la valeur communiquee pour votre integration.</div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="form-text">URL de notification a configurer dans votre tableau de bord CinetPay.</div>
                    </div>
    ...
    <div class="col-xl-5">
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white border-bottom">
                <h6 class="mb-0">Etat de la connexion</h6>
            </div>
            <div class="card-body">
                @if(!empty($config['merchant_id']) && !empty($config['secret_key']))
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <div class="fw-semibold text-success">Configure</div>
                            <small class="text-muted">CinetPay est pret a traiter des paiements.</small>
                        </div>
                    </div>
                @else
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <div class="fw-semibold text-warning">Non configure</div>
                            <small class="text-muted">Renseignez le Merchant ID et la cle secrete.</small>
                        </div>
                    </div>
                @endif
            </div>
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white border-bottom">
                <h6 class="mb-0">Methodes de paiement supportees</h6>
            </div>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white border-bottom">
                <h6 class="mb-0">URL a configurer dans CinetPay</h6>
            </div>
            <div class="card-body">
                <p class="text-muted small mb-2">Copiez cette URL dans votre tableau de bord CinetPay :</p>
                <div class="mb-2">
                    <label class="form-label small fw-semibold">Callback (Webhook)</label>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var defaultCb = '{{ route('api.eshop360.cinetpay.callback') }}';
