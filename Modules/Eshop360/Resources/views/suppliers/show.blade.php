<x-dashboard::layouts.master
    :title="'Supplier — ' . ($supplier->name ?? '') . ' — ' . ($instance->name ?? $instance->slug ?? 'B360')"
    :instance="$instance"
...
<div class="page-header">
    ...
    <div class="page-btn">
        <a href="{{ route('eshop360.suppliers.statement', [$instance->slug ?? '', $supplier]) }}" class="btn btn-info me-2"><i class="ti ti-file-text me-1"></i>Statement</a>
        <a href="{{ route('eshop360.suppliers.edit', [$instance->slug ?? '', $supplier]) }}" class="btn btn-warning me-2"><i data-feather="edit" class="me-1"></i>Edit</a>
    ...
    <div class="col-md-8">
        <ul class="nav nav-tabs" id="supplierTabs" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="purchases-tab" data-bs-toggle="tab" data-bs-target="#purchases-pane" type="button" role="tab">Purchase History</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="imports-tab" data-bs-toggle="tab" data-bs-target="#imports-pane" type="button" role="tab">Import Orders</button>
            </li>
        </ul>
                ...
                <div class="card border-top-0 rounded-top-0">
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table">
                                <thead><tr><th>Reference</th><th>Date</th><th>Total</th><th>Paid</th><th>Due</th><th>Status</th></tr></thead>
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><span class="badge bg-{{ ($purchase->status ?? '') === 'received' ? 'success' : 'warning' }}">{{ ucfirst($purchase->status ?? 'pending') }}</span></td>
                                    </tr>
                                    @empty
            ...
            <div class="tab-pane fade" id="imports-pane" role="tabpanel">
                <div class="card border-top-0 rounded-top-0">
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table">
                                <thead><tr><th>Reference</th><th>Date</th><th>Status</th><th>Shipping</th><th>Total</th></tr></thead>
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><span class="badge bg-{{ $import->status === 'received' ? 'success' : ($import->status === 'shipped' ? 'info' : 'warning') }}">{{ ucfirst($import->status) }}</span></td>
                                        <td>{{ ucfirst($import->shipping_type ?? '—') }}</td>
                                        <td>{{ number_format($import->total ?? 0, 2) }}</td>
                                    </tr>
                                    @empty
                                    <tr><td colspan="5" class="text-center text-muted">No import orders found.</td></tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
