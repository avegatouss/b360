<x-dashboard::layouts.master
    :title="__('Statement') . ' —' . ($supplier->name ?? '') . ' — ' . ($instance->name ?? $instance->slug ?? 'B360')"
    :instance="$instance"
    :pageTitle="__('Supplier Statement')">

<div class="page-header">
    <div class="add-item d-flex">
        <div class="page-title">
            <h4 class="fw-bold">{{ __('Supplier Statement') }}</h4>
            <h6>{{ $supplier->name }} — {{ $supplier->company ?? '' }}</h6>
        </div>
    </div>
    <div class="page-btn">
        <a href="#" onclick="window.print()" class="btn btn-info me-2"><i class="ti ti-printer me-1"></i>{{ __('Print') }}</a>
...
<div class="card">
    <div class="card-header">
        <form action="{{ route('eshop360.suppliers.statement', [$instance->slug ?? '', $supplier]) }}" method="GET" class="row g-3 align-items-end">
            ...
            <div class="col-md-3">
                <button type="submit" class="btn btn-primary"><i class="ti ti-filter me-1"></i>{{ __('Filter') }}</button>
        ...
        <div class="row mb-4">
            <div class="col-md-3">
                <div class="border rounded p-3 text-center">
                    <h6 class="text-muted mb-1">{{ __('Total Purchases') }}</h6>
            ...
            <div class="col-md-3">
                <div class="border rounded p-3 text-center">
                    <h6 class="text-muted mb-1">{{ __('Balance Due') }}</h6>
            ...
            <table class="table table-striped">
                ...
                <tbody>
                    <tr class="table-light">
                        <td colspan="6" class="fw-bold">{{ __('Opening Balance') }}</td>
                    ...
                    @endforelse
                    <tr class="table-dark">
                        <td colspan="4" class="fw-bold">{{ __('Closing Balance') }}</td>
