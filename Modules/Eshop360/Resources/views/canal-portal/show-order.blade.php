<x-dashboard::layouts.master
    :title="__('eshop360::eshop.order') . ' ' . ($onlineOrder->reference ?? $onlineOrder->id) . ' - ' . $channel->name . ' - ' . ($instance->name ?? $instance->slug ?? 'B360')"
    :instance="$instance"
    :pageTitle="__('eshop360::eshop.portal_order_detail') . ' - ' . $channel->name">
...
<div class="page-header">
    <div class="page-title me-auto">
        <h4 class="fw-bold">{{ __('eshop360::eshop.order') }} {{ $onlineOrder->reference }} &mdash; {{ $channel->name }}</h4>
        <h6>{{ $onlineOrder->created_at->format('Y-m-d H:i') }}</h6>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('eshop360.canal-portal.orders.index', [$slug, $channelKey]) }}" class="btn btn-secondary">{{ __('eshop360::eshop.portal_back_to_orders') }}</a>
        <a href="{{ route('eshop360.canal-portal.catalog', [$slug, $channelKey]) }}" class="btn btn-outline-primary">{{ __('eshop360::eshop.portal_catalog') }}</a>
    </div>
</div>
...
<div class="row g-4">
    <div class="col-lg-4">
        <div class="card">
            ...
            <div class="card-body">
                <table class="table table-borderless mb-0">
                    ...
                    <tr><th>{{ __('Canal') }}</th><td>{{ $channel->name }}</td></tr>
        ...
        @if($onlineOrder->status === 'delivered')
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('eshop360.canal-portal.orders.received', [$slug, $channelKey, $onlineOrder]) }}">
                        @csrf
                        @method('PATCH')
        ...
        @if(in_array($onlineOrder->status, ['pending_validation', 'validated'], true))
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('eshop360.canal-portal.orders.cancel', [$slug, $channelKey, $onlineOrder]) }}" onsubmit="return confirm('{{ __('eshop360::eshop.portal_cancel_order_confirmation') }}')">
                        @csrf
                        @method('PATCH')
