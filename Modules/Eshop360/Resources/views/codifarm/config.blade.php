<x-dashboard::layouts.master
    :title="__('Codifarm Configuration') . ' —' . ($instance->name ?? $instance->slug ?? 'B360')"
    :instance="$instance"
    :pageTitle="__('Codifarm Configuration')">

<div class="page-header">
    <div class="add-item d-flex">
        <div class="page-title">
            <h4 class="fw-bold">{{ __('Codifarm Configuration') }}</h4>
            <h6>{{ __('Configure rates and revenue shares') }}</h6>
        </div>
    </div>
    <div class="page-btn">
        ...
        <form action="{{ route('eshop360.codifarm.config.update', $instance->slug ?? '') }}" method="POST">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <label class="form-label">{{ __('Margin Rate (%)') }}<span class="text-danger">*</span></label>
                    ...
                    <small class="text-muted">{{ __('Percentage of margin applied on each order.') }}</small>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">{{ __('Codifarm Share (%)') }}<span class="text-danger">*</span></label>
                    ...
                    <small class="text-muted">{{ __('Codifarm\'s share of the total margin.') }}</small>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">{{ __('Saphir Share (%)') }}<span class="text-danger">*</span></label>
                    ...
                    <small class="text-muted">{{ __('Saphir\'s share of the total margin.') }}</small>
                </div>
                <div class="col-md-6 mb-3">
                    ...
                    <small class="text-muted">{{ __('Commission rate for sales agents.') }}</small>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">{{ __('Debt Threshold (XAF)') }}</label>
                    ...
                    <small class="text-muted">{{ __('Alert threshold for outstanding debt.') }}</small>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">{{ __('Settlement Period (days)') }}</label>
                    ...
                    <small class="text-muted">{{ __('Number of days for settlement cycle.') }}</small>
                </div>
