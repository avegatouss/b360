...
class ReportService
{
    ...
    public function salesByCategory(int $instanceId, string $from, string $to): array
    {
        ...
        return DB::table('eshop_order_items as oi')
            ...
            ->map(fn ($row): array => [
                ...
                'name' => (string) ($row->category ?? 'Sans categorie'),
                'quantity' => (int) ($row->total_qty ?? 0),
                'revenue' => round((float) ($row->total_amount ?? 0), 2),
            ])
            ->all();
    ...
    public function cashbook(int $instanceId, string $from, string $to): array
    {
        ...
        $entries = $payments
            ...
            ->concat($expenses->map(fn (Expense $expense): array => [
                ...
                'description' => (string) ($expense->description ?? $expense->category?->name ?? 'Depense'),
                'method' => $expense->account?->name ?? 'depense',
                'credit' => 0.0,
        ...
        $paymentsByMethod = $payments
            ->groupBy(fn (Payment $payment): string => (string) ($payment->method ?? 'inconnu'))
            ->map(fn ($group): float => round((float) $group->sum('amount'), 2))
            ->toArray();

        $expensesByCategory = $expenses
            ->groupBy(fn (Expense $expense): string => (string) ($expense->category?->name ?? 'Non classe'))
            ->map(fn ($group): float => round((float) $group->sum('amount'), 2))
            ->toArray();
        ...
        $byCashier = $collection
            ...
            ->map(function ($group, string $name): array {
                ...
                return [
                    ...
                    'revenue' => $revenue,
                    'average' => round($transactions > 0 ? $revenue / $transactions : 0, 2),
                ];
            })
            ->values()
            ->all();
    ...
    private function paymentDescription(Payment $payment): string
    {
        ...
        if ($payableReference) {
            return 'Paiement ' . $payableReference;
        }

        return (string) ($payment->notes ?: 'Paiement');
    }
