...
class AccountController extends Controller
{
    ...
    public function index()
    {
        ...
        $accounts = Account::where('instance_id', $instance->id)->get();
        $totalBalance = $accounts->sum('balance');
        return view('eshop360::finance.accounts.index', compact('accounts', 'totalBalance'));
    }
    ...
    public function show(string $slug, Account $account)
    {
        $transactions = AccountTransaction::where('account_id', $account->id)
            ->with('user')
            ->latest()
            ->paginate(30);
        return view('eshop360::finance.accounts.show', compact('account', 'transactions'));
    }
    ...
    public function deposit(Request $request, string $slug, Account $account)
    {
        ...
        return redirect()->back()->with('success', 'Deposit recorded.');
    }
