...
<x-dashboard::layouts.master
    :title="'Instances — ' . ($instance->name ?? $instance->slug ?? 'B360')"
    :instance="$instance"
    pageTitle="Gestion des modules"
    :breadcrumbs="[
        ['label' => 'Liste des modules', 'url' => route('modules.index', ['slug' => $instance->slug])],
    ]"
>
                    ...
                    <div class="d-flex gap-2">
                        ...
                        @if(!$mod->is_protected)
                            <form method="POST" action="{{ route('modules.toggle', [$instance->slug, $mod->name]) }}">
                                @csrf
                                @method('PUT')
                                <button class="btn {{ $mod->is_enabled ? 'btn-outline-warning' : 'btn-outline-success' }}">
                                    <i class="fas {{ $mod->is_enabled ? 'fa-pause' : 'fa-play' }} me-1"></i>
