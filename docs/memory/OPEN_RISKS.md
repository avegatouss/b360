...
- **Sous-lots** :
  ...
  - ✅ **S6 — Promotions** (2026-04-24) : 5 modèles déplacés sous `Domain/Promotions/Models/` (Coupon, Discount, DiscountPlan, GiftCard, GiftCardTopup). 5 stubs d'alias créés. Deptrac `EshopPromotions` restreint à socles + `EshopCRM` (pour GiftCard→Customer) + Eshop360 transitoire. Pas de dépendance Catalog (Discount.product_ids est JSON, pas relation typée). ADR-014. 0 régression.
  - ✅ **S7 — Purchasing** (2026-04-24) : 9 modèles déplacés sous `Domain/Purchasing/Models/` (Supplier, PurchaseOrder, PurchaseItem, PurchaseReturn, PurchaseReturnItem, ImportOrder, ImportOrderItem, ImportCost, ImportCostType). 9 stubs d'alias créés. Deptrac `EshopPurchasing` restreint à socles + `EshopCatalog` (Product) + `EshopInventory` (Warehouse/Store) + Eshop360 transitoire (Payment à basculer en S9). ADR-015. 0 régression.
  - ✅ **S8 — Sales** (2026-04-24, **L1**) : 9 modèles déplacés sous `Domain/Sales/Models/` (Order, OrderItem, OnlineOrder, OnlineOrderItem, PersistentCart, SaleReturn, CashRegister, Quotation, QuotationItem). 9 stubs d'alias créés. **Mesure défensive `$morphClass` pinning** sur chaque canonique → garantit stabilité des FQN stockées dans `reference_type`/`payable_type`/`invoiceable_type` malgré le déplacement, indépendamment des imports consumer. Deptrac `EshopSales` restreint à socles + Catalog + CRM + Channel + Inventory + Eshop360 transitoire. Baseline PHPStan 3656 (+9 `missingType.property` pour les `$morphClass` — bruit baseliné). ADR-016. 0 régression.
  - ✅ **S9 — Finance** (2026-04-24, **L1**) : **20 modèles** déplacés sous `Domain/Finance/Models/` (Invoice, InvoiceItem, RecurringInvoice, Payment, PaymentMethod, EshopPaymentGateway, Account, AccountTransaction, AccountTransfer, Expense, ExpenseCategory, Income, IncomeSource, ChargeCategory, ChargeLog, CompanyCharge, InstallmentPayment, InstallmentPlan, LoanPayment, FneInvoice) + 20 alias stubs + `$morphClass` pinning universel. Fix covariance InvoiceService::syncPaidAmount (return type migré vers canonique Payment). Deptrac `EshopFinance` restreint à socles + Catalog + CRM + Sales + Eshop360 transitoire. Baseline PHPStan 3682 (+26 vs S8). ADR-017. 0 régression.
  - ✅ **S10 — HR** (2026-04-24) : 4 modèles déplacés sous `Domain/HR/Models/` (Employee, EmployeeCommission, EmployeeSalary, Attendance) + 4 alias stubs + `$morphClass` pinning. 1 seul import cross-subdomain (EmployeeCommission → Order). Deptrac `EshopHR` restreint à socles + `EshopSales` + Eshop360 transitoire. Baseline PHPStan 3686 (+4 vs S9). ADR-018. 0 régression.
  - ✅ **S11 — Communication + Projects + Reporting + rattrapages** (2026-04-24) : **20 modèles** déplacés — 11 Communication (EmailTemplate, BulkMessageLog, Message, SmsGateway, SmsLog, SupportTeam, SupportTicket, TicketMessage, ReceiptTemplate, Webhook, WebhookLog), 4 Projects (Project, Task, TaskComment, Event), 2 Reporting (ApiLog, AuditLog), 3 rattrapages (Holding → Sales, Loan + LoanSchedule → Finance). 20 alias stubs + `$morphClass` pinning universel. 3 nouvelles rulesets deptrac restreintes (Comm: socles+CRM+Inventory ; Proj: socles+CRM+Sales+Finance ; Report: socles uniquement). **Extraction physique R-101 = 13/13 (100 %)**. Baseline PHPStan 3704. ADR-019. 0 régression.
  - ⏳ **S12 — Clôture R-101** (à venir) : suppression des ~90 alias stubs, formalisation morph map via `enforceMorphMap`, bascule imports alias → FQN canonique dans services/controllers, levée des dépendances transitoires `EshopX → Eshop360`.
